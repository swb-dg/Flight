...
- Streamlit 기반 사용자 입력 UI 완성 (`app.py`)
- 입력 항목 구성:
  - 출발/도착 공항: GMP, ICN, CJU만 허용 (`selectbox`)
  - 탑승일시: 시작일/종료일 + 오전/오후 (`date_input` + `selectbox`)
  - 이메일 입력 (`text_input`), `@` 포함 여부만 간단 검증
  - 등록 버튼 → 입력값 `st.session_state`에 저장
- 자동 반전 처리: 시작일/시간 > 종료일/시간이면 자동으로 순서 교체
- Streamlit UI 정상 실행 및 반응형 레이아웃 구성 확인
- Git 커밋: `"feat: 사용자 입력 UI 구성"`  
  작업 브랜치: `dev`

### 🧠 기억 보존 & 기준 유지 메모

- `app.py`는 전체 UI의 단일 진입점으로 유지
- 이후 `user_config.py`에서 `st.session_state["user_input"]` 값을 구조화해서 사용할 예정
- UI 고정 상태이므로 이후 단계에서 UI 수정 없이 그대로 사용
- UI 항목 구조는 추후 기능 확장 (예: 공항 추가, 시간 범위 세분화) 시에도 호환되도록 설계됨

## 3단계 메모

### ✅ 정리 요약

- `user_config.py` 파일 생성 및 `get_user_config()` 함수 구현 완료
- 주요 처리 기능:
  - 입력값 필수 항목 확인
  - 출발/도착 공항 동일 여부 체크
  - 이메일 형식 간단 검증 (`@` 포함 여부)
  - 날짜 + 시각 → datetime 객체로 변환
  - 시작일 > 종료일인 경우 자동으로 반전 처리
- 결과물은 아래 형태의 dict 반환:
```python
{
    "departure": "GMP",
    "arrival": "CJU",
    "start_datetime": datetime(2025, 4, 25, 6, 0),
    "end_datetime": datetime(2025, 4, 26, 18, 0),
...
- `get_user_config()`는 모든 후속 모듈(check_ticket, send_email_alert 등)에서 입력의 단일 진입점 역할을 함
- UI에서 시간 입력 방식이 `time_input`으로 바뀌었으므로, 더 이상 "오전/오후" 구분을 사용하지 않음
- datetime 결합 기준:
...
- `requirements.txt`, `README.md`, `.streamlit/config.toml` 작성 및 저장소 반영 준비 완료
- Streamlit Cloud 배포를 위한 최종 파일 구성:
  - `app.py`: Streamlit UI 메인 앱
  - `user_config.py`, `ticket_checker.py`, `email_sender.py`, `monitor.py`
  - `requirements.txt`, `README.md`, `.streamlit/config.toml`
- GitHub 푸시 후, Streamlit Cloud에서 배포 진행:
  - https://streamlit.io/cloud 접속 → GitHub 저장소 연결 → `app.py` 선택 → Deploy

### 🧠 기억 보존 & 기준 유지 메모

- 배포 시 기본 브랜치는 `main`, 진입 파일은 `app.py`로 고정
- `requirements.txt`는 버전 지정 포함 (`streamlit>=1.32`, `requests>=2.31`)
- 향후 사용자 지정 주기, 다중 사용자 확장, 로그인 기반 설정 저장 기능으로 진화 가능
