...
- 입력 항목 구성:
  ...
  - 탑승일시: 시작일/종료일 + **시작/종료 시각** (`date_input` + `time_input`)
    - 초기에는 오전/오후로 구성되었으나, 3단계에서 조건 필터링의 정밀성을 위해 시각 단위(`time_input`)로 변경
...
- UI 고정 상태이므로 이후 단계에서 구조만 유지하며 내부 처리 방식 변경 가능
- UI 항목 구조는 추후 기능 확장 (예: 공항 추가, 시간 범위 세분화) 시에도 호환되도록 설계됨
- **시간대 입력은 `time_input`으로 고정되었으며**, 필터링 로직 정확성을 고려해 오전/오후 선택 방식은 폐기됨## ✅ 정리 요약
